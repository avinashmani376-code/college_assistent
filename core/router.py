...
from core.intent import classify_intent, detect_language, is_detail_request, is_basic_knowledge
...
def _search_answer(user_message: str, history, lang: str, detailed: bool) -> str:
    """
    Route:
      - Basic knowledge (AI, Solar System, Modi, etc.) → Groq directly (fast, no Tavily).
      - Current/recent topics → Tavily → Wikipedia/DDG → pure AI.
    ONE AI call at most.
    """

    # Step 0: Basic/stable knowledge — skip Tavily entirely
    if is_basic_knowledge(user_message):
        print(f"[SEARCH] Basic knowledge detected — skipping Tavily, using Groq directly")
        return query_ai(prompt=user_message, history=history, lang=lang,
                        mode="general", detailed=detailed)

    # Step 1: Tavily — only for current/recent information
    ...
    if ctx:
        ...
        if lang == "te":
            ...
            prompt = (
                ...
                + ("Explain thoroughly." if detailed else "Answer in 1-2 sentences only.")
            )
    ...
    try:
        ...
        if intent == "news":
            ...
            try:
                ...
                logger.exception("[ROUTE] news fetch failed: %s", news_exc)
                print(f"[ROUTE] news exception: {news_exc}")
                reply = ("వార్తలు తీసుకోలేకపోయాను. తర్వాత మళ్లీ ప్రయత్నించండి."
                         if lang == "te"
                         else "Unable to fetch news right now. Please try again later.")
        ...
        # ── SEARCH / GENERAL — basic knowledge → Groq; recent → Tavily → Web → AI ──
...
def news_sidebar():
    try:
        print("[NEWS-SIDEBAR] fetching sidebar articles")
        articles, provider = fetch_news("india education students latest")
        print(f"[NEWS-SIDEBAR] provider={provider} count={len(articles)}")
        cleaned = [
            {
        ...
        print(f"[NEWS-SIDEBAR] returning {len(cleaned)} cleaned articles")
        return jsonify({"articles": cleaned})
    except Exception as exc:
        logger.exception("[NEWS-SIDEBAR] failed: %s", exc)
